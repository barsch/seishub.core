...
from seishub.config import default_dir
...
def load_components(env):
    ...
    if pkg_resources is not None:
        ...
        for egg in flatten([pkg_env[name][0] for name in pkg_env]):
            ...
            for name in egg.get_entry_map('seishub.plugins'):
                # Load plugins declared via the `seishub.plugins` entry point.
                ...
                egg.activate()
                try:
                    entry_point = egg.get_entry_info('seishub.plugins', name)
