...
class ProcessorMOVETestSuite(SeisHubEnvironmentTestCase):
    ...
    def test_moveWithoutDestinationHeader(self):
        ...
        proc.run(PUT, '/xml/move-test/notvc/test.xml', StringIO(XML_DOC))
        try:
            proc.run(MOVE, '/xml/move-test/notvc/test.xml')
            self.fail("Expected SeisHubError")
        except SeisHubError, e:
            self.assertEqual(e.code, http.BAD_REQUEST)
        # remove resource
        proc.run(DELETE, '/xml/move-test/notvc/test.xml')
    ...
    def test_moveWithOversizedDestination(self):
        ...
        proc = Processor(self.env)
        proc.run(PUT, '/xml/move-test/notvc/test.xml', StringIO(XML_DOC))
        uri = self.env.getRestUrl() + '/xml/move-test/notvc/' + \
              'a' * (MAX_URI_LENGTH + 1)
        try:
            proc.run(MOVE, '/xml/move-test/notvc/test.xml', 
    ...
    def test_moveToOtherResourceType(self):
        ...
        uri = self.env.getRestUrl() + '/xml/move-test/vc/test2.xml'
        try:
            proc.run(MOVE, '/xml/move-test/notvc/test.xml', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
    ...
    def test_moveWithoutFilename(self):
        ...
        uri = self.env.getRestUrl() + '/xml/move-test/vc/'
        try:
            proc.run(MOVE, '/xml/move-test/notvc/test.xml', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
    ...
    def test_moveToNewResource(self):
        ...
        proc.run(PUT, '/xml/move-test/notvc/test.xml', StringIO(XML_DOC))
        # move
        uri = self.env.getRestUrl() + '/xml/move-test/notvc/new.xml'
        data = proc.run(MOVE, '/xml/move-test/notvc/test.xml', 
                        received_headers = {'Destination': uri})
        ...
        try:
            proc.run(GET, '/xml/move-test/notvc/test.xml')
            self.fail("Expected SeisHubError")
        except SeisHubError, e:
            self.assertEqual(e.code, http.NOT_FOUND)
        # get new resource
        data = proc.run(GET, '/xml/move-test/notvc/new.xml')
        self.assertEqual(data, XML_DOC)
        # revert move
        uri = self.env.getRestUrl() + '/xml/move-test/notvc/test.xml'
        proc.run(MOVE, '/xml/move-test/notvc/new.xml', 
                 received_headers = {'Destination': uri})
        # get original resource
        proc.run(GET, '/xml/move-test/notvc/test.xml')
        # get new resource 
        try:
            proc.run(GET, '/xml/move-test/notvc/new.xml')
            self.fail("Expected SeisHubError")
        except SeisHubError, e:
            self.assertEqual(e.code, http.NOT_FOUND)
        # remove resource
    ...
    def test_moveRevisionToRevision(self):
        ...
        proc = Processor(self.env)
        # create resource
        proc.run(PUT, '/xml/move-test/vc/test.xml', StringIO(XML_DOC))
        proc.run(POST, '/xml/move-test/vc/test.xml', StringIO(XML_DOC))
        proc.run(POST, '/xml/move-test/vc/test.xml', StringIO(XML_DOC))
        proc.run(POST, '/xml/move-test/vc/test.xml', StringIO(XML_DOC))
        # try to move revision 2 to revision 1 
        uri = self.env.getRestUrl() + '/move-test/vc/test.xml/1'
        try:
            proc.run(MOVE, '/xml/move-test/vc/test.xml/2', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
        except SeisHubError, e:
            self.assertEqual(e.code, http.FORBIDDEN)
        # delete resource
        proc.run(DELETE, '/xml/move-test/vc/test.xml')
    ...
    def test_moveRevisionToNewResource(self):
        ...
        try:
            proc.run(MOVE, '/xml/move-test/vc/test.xml/1', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
        except SeisHubError, e:
            self.assertEqual(e.code, http.FORBIDDEN)
    ...
    def test_moveToExistingRevision(self):
        ...
        proc = Processor(self.env)
        # create resources
        proc.run(PUT, '/xml/move-test/vc/test1.xml', StringIO(XML_DOC))
        proc.run(PUT, '/xml/move-test/vc/test2.xml', StringIO(XML_DOC))
        proc.run(POST, '/xml/move-test/vc/test2.xml', StringIO(XML_DOC))
        proc.run(POST, '/xml/move-test/vc/test2.xml', StringIO(XML_DOC))
        proc.run(POST, '/xml/move-test/vc/test2.xml', StringIO(XML_DOC))
        # try to overwrite the existing revision 1
        uri = self.env.getRestUrl() + '/xml/move-test/vc/test2.xml/1'
        try:
            proc.run(MOVE, '/xml/move-test/vc/test1.xml', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
        except SeisHubError, e:
            self.assertEqual(e.code, http.FORBIDDEN)
        # delete resources
        proc.run(DELETE, '/xml/move-test/vc/test1.xml')
        proc.run(DELETE, '/xml/move-test/vc/test2.xml')
    ...
    def test_moveToNewRevision(self):
        ...
        uri = self.env.getRestUrl() + '/xml/move-test/vc/test2.xml/4'
        try:
            proc.run(MOVE, '/xml/move-test/vc/test1.xml', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
    ...
    def test_moveToExistingResource(self):
        ...
        proc = Processor(self.env)
        # create resources
        proc.run(PUT, '/xml/move-test/notvc/test1.xml', StringIO(XML_DOC))
        proc.run(PUT, '/xml/move-test/notvc/test2.xml', StringIO(XML_DOC))
        # try to overwrite test2.xml 
        uri = self.env.getRestUrl() + '/xml/move-test/notvc/test2.xml'
        try:
            proc.run(MOVE, '/xml/move-test/notvc/test1.xml', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
        except SeisHubError, e:
            self.assertEqual(e.code, http.CONFLICT)
        # delete resources
        proc.run(DELETE, '/xml/move-test/notvc/test1.xml')
        proc.run(DELETE, '/xml/move-test/notvc/test2.xml')
    ...
    def test_moveBuiltinResource(self):
        ...
        uri = '/xml/seishub/stylesheet/' + data.keys()[0]
        to_uri = self.env.getRestUrl() + '/xml/seishub/stylesheet/test.xml' 
        try:
    ...
    def test_moveToInvalidResourcename(self):
        ...
        proc.run(PUT, '/xml/move-test/notvc/test.xml', StringIO(XML_DOC))
        # starting tilde (~)
        uri = self.env.getRestUrl() + '/xml/move-test/notvc/~test'
        try:
            proc.run(MOVE, '/xml/move-test/notvc/test.xml', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
        ...
        uri = self.env.getRestUrl() + '/xml/move-test/notvc/_test'
        try:
            proc.run(MOVE, '/xml/move-test/notvc/test.xml', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
        ...
        try:
            ...
            self.assertEqual(e.code, http.BAD_REQUEST)
        # but this should go
        uri = self.env.getRestUrl() + '/xml/move-test/notvc/Aaz09_-.xml'
        proc.run(MOVE, '/xml/move-test/notvc/test.xml', 
                 received_headers = {'Destination': uri})
        # and revert again
        uri = self.env.getRestUrl() + '/xml/move-test/notvc/test.xml'
        proc.run(MOVE, '/xml/move-test/notvc/Aaz09_-.xml', 
                 received_headers = {'Destination': uri})
        proc.run(DELETE, '/xml/move-test/notvc/test.xml')
    ...
    def test_moveToDifferentServer(self):
        ...
        proc = Processor(self.env)
        proc.run(PUT, '/xml/move-test/notvc/test.xml', StringIO(XML_DOC))
        uri = 'http://somewhere:8080/xml/move-test/notvc/test.xml'
        try:
            proc.run(MOVE, '/xml/move-test/notvc/test.xml', 
                     received_headers = {'Destination': uri})
            self.fail("Expected SeisHubError")
