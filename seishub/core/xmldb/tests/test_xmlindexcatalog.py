...
#        after = list(self.catalog._cache['package_id'].values()[0])
    ...
    def test_queryIndexView(self):
        # create test catalog
        self._setup_testdata()
        self.catalog.updateIndexView(self.idx1)
        # parse input arguments
        tab = Table('/testpackage/station', self.env.db.metadata,
        ...
        query = sql.select(columns, distinct=True,
                           order_by=[tab.c['resource_name']])
        result = self.env.db.query(query).first()
        self.assertEquals(result['package_id'], 'testpackage')
        self.assertEquals(result['resourcetype_id'], 'station')
        self.assertEquals(result['resource_name'], 'RAW_XML1')
        # remove test catalog
        self._cleanup_testdata()
